...
LnxReadCmdSet *lnx_tag_sort(UT_array *tags, int maxByte) {
    ...
    for (uint16_t i = 0; i < result->n_sort; i++) {
        LnxPoint *tag = *(LnxPoint **) utarray_front(result->sorts[i].tags);
        struct LnxSortCtx *ctx = result->sorts[i].info.context;

        sortResult->cmd[i].tags = utarray_clone(result->sorts[i].tags);
        sortResult->cmd[i].ipAddr = tag->ipAddr;
        sortResult->cmd[i].command = tag->command;
        sortResult->cmd[i].msg = tag->msg;
