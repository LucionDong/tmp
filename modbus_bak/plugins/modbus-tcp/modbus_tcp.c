...
static int driver_init(neu_plugin_t *plugin, bool load) {
    ...
    plugin->stack = modbus_stack_create((void *) plugin, MODBUS_PROTOCOL_TCP, modbus_send_msg, modbus_value_handle,
                                        modbus_write_resp);
...
static int driver_config(neu_plugin_t *plugin, const char *config) {
    ...
    if (plugin->conn != NULL) {
        ...
        plugin->conn = neu_conn_new(&param, (void *) plugin, modbus_conn_connected, modbus_conn_disconnected);
    }
...
static int driver_validate_tag(neu_plugin_t *plugin, neu_datatag_t *tag) {
    ...
    if (ret == 0) {
        plog_notice(plugin,
                    ...
                    tag->name, tag->address, tag->type, point.slave_id, point.start_address, point.n_register,
