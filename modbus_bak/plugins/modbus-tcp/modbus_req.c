...
{
	printf("modbus_tcp_server_listen\n");
    struct neu_plugin   *plugin = (struct neu_plugin *) data;
...
int modbus_tcp_server_io_callback(enum neu_event_io_type type, int fd,
                                  void *usr_data)
{
	printf("modbus_tcp_server_io_callback\n");
                       ...
                       uint16_t max_byte)
{
    ...
    for (uint16_t i = 0; i < gd->cmd_sort->n_cmd; i++) {
        ...
        if (ret_r > 0) {
            ...
            //             plog_warn(plugin,
            //                       "no modbus response received, skip,
            //                       %hhu!%hu", gd->cmd_sort->cmd[i].slave_id,
            //                       gd->cmd_sort->cmd[i].start_address);
            //         }
            //     } else if (ret_buf < 0) {
            //         if (ret_buf == -1) {
            ...
            //                                 0, NULL,
            //                                 NEU_ERR_PLUGIN_PROTOCOL_DECODE_FAILURE);
            //             plog_error(plugin, "modbus message error, skip,
            //             %hhu!%hu",
            //                        gd->cmd_sort->cmd[i].slave_id,
            //                        gd->cmd_sort->cmd[i].start_address);
            //         } else if (ret_buf == -2) {
            ...
            //     for (uint16_t j = 0; j < plugin->max_retries; j++) {
            //         ret_r =
            //             modbus_stack_read_retry(plugin, gd, i, j,
            //             &response_size);
            //         if (ret_r > 0) {
            //             ret_buf = process_protocol_buf(
            //                 plugin, gd->cmd_sort->cmd[i].slave_id,
            ...
            //                 modbus_value_handle(
            //                     plugin, gd->cmd_sort->cmd[i].slave_id, 0,
            //                     NULL, NEU_ERR_PLUGIN_DEVICE_NOT_RESPONSE);
            //                 plog_warn(plugin,
            //                           "no modbus response received, skip,
            //                           %hhu!%hu",
            //                           gd->cmd_sort->cmd[i].slave_id,
